...
import static it.unibo.conversational.Utils.tokenSimilarity;
...
//    assertEquals(0.6, n4.minSimilarity(), PRECISION);
    ...
    public void testTokenSimilarity() {
        ...
        assertEquals(1, tokenSimilarity(string2ngram("= a"), string2ngram("a a")), 0.01); // HOWEVER THE SHOULD BE 0.5
        assertEquals(jacc.d("customer", "all customer"), jacc.d("customer", "customer id"));
        assertTrue(tokenSimilarity(string2ngram("customer"), string2ngram("customer id")) > tokenSimilarity(string2ngram("customer"), string2ngram("all customer")));
        ...
        assertEquals(0.875, tokenSimilarity("cutomer", "customer"), PRECISION);
        assertEquals(0.4, tokenSimilarity("cost", "store_cost"), PRECISION);
        assertEquals(0.272, tokenSimilarity("sum store", "store sales"), PRECISION);
        assertEquals(0.636, tokenSimilarity("store sum", "store sales"), PRECISION);
        assertEquals(0.4, tokenSimilarity("cost", "store cost"), PRECISION);
        assertEquals(0.666, tokenSimilarity("media", "medium"), PRECISION);
        assertEquals(0.5, tokenSimilarity("media", "media type"), PRECISION);
        assertEquals(0.4, tokenSimilarity("type", "store type"), PRECISION);
        assertEquals(0.4, tokenSimilarity("type", "media type"), PRECISION);
        assertEquals(0.636, tokenSimilarity("media based", "media type"), PRECISION);
        assertEquals(0.09, tokenSimilarity("based media", "media type"), PRECISION);
        assertEquals(1, tokenSimilarity("media type", "media type"), PRECISION);
        assertEquals(0.5, tokenSimilarity("the_year", "year"), PRECISION);
        assertEquals(0.428, tokenSimilarity("greater", "year"), PRECISION);
        assertEquals(0.4, tokenSimilarity("cost", "store cost"), PRECISION);
        assertEquals(0.199, tokenSimilarity("store cost", "cost by"), PRECISION);
        assertEquals(0.199, tokenSimilarity("cost by", "store cost"), PRECISION);
        assertEquals(0.5, tokenSimilarity("year", "the year"), PRECISION);
        assertEquals(0.625, tokenSimilarity("for year", "the year"), PRECISION);
        assertEquals(0.1875, tokenSimilarity("average store cost".split(" "), new String[]{"avg"}), PRECISION);
        assertEquals(0.4375, tokenSimilarity("average store cost".split(" "), new String[]{"average"}), PRECISION);
        assertEquals(0.583, tokenSimilarity("average store".split(" "), "store_state".split("_")), PRECISION);
        assertEquals(0.571, tokenSimilarity("for year".split(" "), "the year".split(" ")), PRECISION);
    }
