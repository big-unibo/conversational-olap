...
public class Ngram implements Serializable {
    ...
    public static double avgSimilarity(final Ngram n) {
        ...
        traverseLeaves(n, (l, acc) -> {
            score.add(l.similarity());
            count.add(1);
            return -1;
        });
    ...
    public Set<Entry<String, Pair<AnnotationType, Set<Entity>>>> getAnnotationsInTree() {
        ...
        Ngram.traverse(this, (c, acc) -> {
            if (!c.getAnnotations().isEmpty()) {
                annotations.addAll(c.getAnnotations().entrySet());
            }
            return -1;
        });
        return annotations;
    ...
    public int countAnnotationsInTree() {
        LongAdder adder = new LongAdder();
        Ngram.traverse(this, (c, acc) -> {
            c.getAnnotations().entrySet().forEach(a -> adder.add(1));
            return -1;
        });
        return adder.intValue();
    }
    ...
    public Object countHintsInTree() {
        LongAdder adder = new LongAdder();
        Ngram.traverse(this, (c, acc) -> {
            if (!c.getHints().isEmpty()) adder.add(1);
            return -1;
        });
        return adder.intValue();
    }
