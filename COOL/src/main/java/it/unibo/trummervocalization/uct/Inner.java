...
public class Inner extends UctNode {
    ...
    public Optional<UctNode> nextNode(long timeout) {
        ...
        while (!maxRewardChild.isPresent()) {
            this.sample();
    ...
    protected double sample() {
        ...
        Optional<UctNode> maxUct = Optional.of(unvisited.orElse(
				this.children
						.stream()
						.filter(c -> c.nVisits > 0)
						.max(Comparator.comparingDouble(c -> c.accReward / c.nVisits + Math.sqrt(2 * Math.log(this.nVisits) / c.nVisits))).get()
		));
